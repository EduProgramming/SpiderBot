...
int motorSpeed = 200;
unsigned long pianoPlayTime = 300;
unsigned long pianoRestTime = 500;
float distance;
char cmd;
...
/**
 * 초음파센서피아노 쉼표
 * @params commandChar, intervalTime
 */
void ultrasonicSensorPianoDelay(char commandChar, unsigned long intervalTime) {
  currentTime = millis();
  while (millis() - currentTime < intervalTime) {
    getBluetoothCommand();
    if (cmd != commandChar) {
      ledControl(false, false, false);
      return;
 ...
 * 초음파센서피아노
 */
void ultrasonicSensorPiano() {
  char commandChar = 'P';
  while (cmd == commandChar) {
    distance = getDistance();
    int distanceInteger = (int) (distance / 7);
    if (distanceInteger < 8) {
      ledControl(PIANO_LED[distanceInteger][0], PIANO_LED[distanceInteger][1], PIANO_LED[distanceInteger][2]);
      tone(PIN_BUZZER, NOTES[distanceInteger], pianoPlayTime);
      ultrasonicSensorPianoDelay(commandChar, pianoRestTime);
    } else {
      ledControl(false, false, false);
    }
    getBluetoothCommand();
